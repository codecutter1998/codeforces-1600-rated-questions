...
void solve()
{
    ...
    rep(i,n)
    {
        auto steps = lower_bound(store,store+n,make_pair(store[i].ff-store[i].ss,-INF))-store-1;
        // int tmp = lower_bound(store, store + n, make_pair(store[i].x - store[i].y, 0)) - store - 1;
        if(steps<0)stepss[i]=1;
        else stepss[i]=stepss[steps]+1;    
        mx=max(mx,stepss[i]);    
    }
    cout<<n-mx; 
}
